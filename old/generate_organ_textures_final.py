#!/usr/bin/env python3
"""
Enhanced realistic organ texture generator with anatomically accurate colon textures.
Creates textures with proper haustra, teniae coli, and realistic surface details.
"""
...
def create_anatomical_vascular_network(size: int, organ_name: str, tissue_props: dict, shape_info: dict) -> np.ndarray:
    ...
    for _ in range(num_main_vessels):
        # Create simple vessel pattern
        if fiber_direction == "longitudinal":
            # Longitudinal vessels
            for i in range(3):
                start_y = random.uniform(0.2, 0.8)
                vessel_width = random.uniform(2, 5)
                for x in range(size):
                    y = int(start_y * size)
                    for dy in range(-int(vessel_width), int(vessel_width) + 1):
                        ny = y + dy
                        if 0 <= ny < size:
                            vascular_mask[ny, x] = 1.0
            ...
            for i in range(3):
                ...
                for y in range(size):
                    x = int(start_x * size)
                    for dx in range(-int(vessel_width), int(vessel_width) + 1):
                        nx = x + dx
                        if 0 <= nx < size:
                            vascular_mask[y, nx] = 1.0
        else:
            # Mixed directions
            for i in range(4):
...
def create_anatomical_surface_pattern(size: int, organ_name: str, tissue_props: dict, shape_info: dict) -> np.ndarray:
    ...
    if tissue_type == "hollow_muscle" and "colon" in organ_name.lower():
        ...
        # Default pattern for other organs
